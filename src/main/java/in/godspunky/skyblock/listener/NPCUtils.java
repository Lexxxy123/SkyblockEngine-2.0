        ...
        if (NPCUtils.HP_CURRENT_NPCMOB.containsKey(npc.getEntity())) {
            if (NPCUtils.HP_MAX_NPCMOB.containsKey(npc.getEntity())) {
                if (NPCUtils.HP_MAX_NPCMOB.get(npc.getEntity()) >= NPCUtils.HP_CURRENT_NPCMOB.get(npc.getEntity())) {
                    ...
                    SLog.severe("NPC STATS ERROR! You can't set Health > Max Health! Cannot set Health >>> " + npc);
                }
            } else {
                SLog.severe("NPC STATS ERROR! Max health is null! Cannot set Health >>> " + npc);
            }
        } else {
            SLog.severe("NPC STATS ERROR! Health is null! >>> " + npc);
        }
    }
