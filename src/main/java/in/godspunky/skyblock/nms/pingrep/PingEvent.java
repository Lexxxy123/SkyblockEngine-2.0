...
public class PingEvent {
    ...
    public ServerInfoPacket createNewPacket(final PingReply reply) {
        try {
            ...
            final Class<?> packet = Class.forName("in.godspunky.skyblock.nms.pingrep." + version + ".ServerInfoPacketHandler");
    ...
    public void sendPong() {
        try {
            final String name = Bukkit.getServer().getClass().getPackage().getName();
            final String version = name.substring(name.lastIndexOf(46) + 1);
            final Class<?> packet = Class.forName("in.godspunky.skyblock.nms.pingrep." + version + ".PongPacketHandler");
            final Constructor<?> constructor = packet.getDeclaredConstructor(this.getClass());
            final PongPacket pong = (PongPacket) constructor.newInstance(this);
            pong.send();
        } catch (final ClassNotFoundException | NoSuchMethodException | SecurityException | InstantiationException |
                       IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
            e.printStackTrace();
