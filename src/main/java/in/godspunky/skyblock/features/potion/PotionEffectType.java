package in.godspunky.skyblock.features.potion;
...
public class PotionEffectType {
    ...
    public static final PotionEffectType SPEED = new PotionEffectType(ChatColor.BLUE + "Speed", "speed", "Grants a %s Speed boost.", PotionColor.LIGHT_BLUE, (statistics, slot, level) -> statistics.getSpeed().add(slot, (double) level.intValue() * 5.0 / 100.0), false, true);
    ...
    public static final PotionEffectType ADRENALINE = new PotionEffectType(ChatColor.RED + "Adrenaline", "adrenaline", "Grants %s Absorption health and an increase of %s Speed.", PotionColor.PURPLE, (effect, player) -> PlayerUtils.replacePotionEffect(player, new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.ABSORPTION, (int) effect.getDuration(), (int) ((double) SUtil.getOrDefault(Arrays.asList(20, 40, 60, 80, 100, 150, 200, 300), effect.getLevel() - 1, Integer.valueOf((effect.getLevel() - 5) * 100)).intValue() * 0.25 - 1.0))), (statistics, slot, level) -> statistics.getSpeed().add(slot, (double) level.intValue() * 5.0 / 100.0), false, true);
    public static final PotionEffectType CRITICAL = new PotionEffectType(ChatColor.DARK_RED + "Critical", "critical", "Gives a %s% increase in Crit Chance and a %s% increase in Crit Damage.", PotionColor.DARK_RED, (statistics, slot, level) -> {
        statistics.getCritChance().add(slot, 0.05 + (double) level.intValue() * 5.0 / 100.0);
    ...
    public static final PotionEffectType RESISTANCE = new PotionEffectType(ChatColor.GREEN + "Resistance", "resistance", "Increases Defense by %s.", PotionColor.DARK_GREEN, (statistics, slot, level) -> statistics.getDefense().add(slot, SUtil.getOrDefault(Arrays.asList(5.0, 10.0, 15.0, 20.0, 30.0, 40.0, 50.0, 60.0), level - 1, Double.valueOf((double) level.intValue() * 10.0 - 20.0))), false, true);
    public static final PotionEffectType TRUE_RESISTANCE = new PotionEffectType(ChatColor.WHITE + "True Resistance", "true_resistance", "Increases True Defense by %s.", PotionColor.TWILIGHT_BLUE, (statistics, slot, level) -> statistics.getTrueDefense().add(slot, (double) level.intValue() * 5.0), false, true);
    public static final PotionEffectType MAGIC_FIND = new PotionEffectType(ChatColor.AQUA + "Magic Find", "magic_find", "Increases Magic Find by %s%.", PotionColor.LIGHT_BLUE, (statistics, slot, level) -> statistics.getMagicFind().add(slot, (double) level.intValue() * 0.02), false, true);
    ...
    public PotionEffectType(final String name, final String namespace, final String description, final PotionColor color, final BiConsumer<PotionEffect, Player> onDrink, final TriConsumer<PlayerStatistics, Integer, Integer> statsUpdate, final boolean instant, final boolean buff) {
        ...
        this.buff = buff;
        PotionEffectType.POTION_EFFECT_TYPE_CACHE.put(namespace, this);
    }
    ...
    public static PotionEffectType getByNamespace(final String namespace) {
        return PotionEffectType.POTION_EFFECT_TYPE_CACHE.get(namespace.toLowerCase());
    }
