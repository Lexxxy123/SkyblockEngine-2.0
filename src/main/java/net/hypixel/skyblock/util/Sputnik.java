...
public class Sputnik {
    ...
    public static void midasCalcDamage(final Entity entity1, final Player player, final SItem sItem) {
        ...
        if (act == "True") {
            ...
            for (final Entity entity2 : entity1.getWorld().getNearbyEntities(entity1.getLocation(), 1.0, 4.0, 1.0)) {
                ...
                if (entity2 instanceof Player || entity2 instanceof EnderDragonPart || entity2 instanceof Villager) {
                    continue;
                }
                ...
                if (!Sputnik.MidasStaff.containsKey(sItem)) {
                    Sputnik.MidasStaff.put(sItem, 0);
                }
                if (!Sputnik.MidasStaffDmg.containsKey(sItem)) {
    ...
    public static void drawLine(final Location point1, final Location point2, final double space) {
        ...
        final Location loc2 = point2.subtract(Math.cos(angle_2), 0.0, Math.sin(angle_2));
    ...
    public static void ferocityParticle(final LivingEntity e) {
        ...
        final Location loc1 = e.getLocation().clone().add(0.0, 1.5, 0.0).add(Math.cos(angle_1), 0.0, Math.sin(angle_1));
        final Location loc2 = e.getLocation().clone().add(0.0, 1.5, 0.0).subtract(Math.cos(angle_1), 0.0, Math.sin(angle_1));
        if (side == 1) {
    ...
    public static void witherShieldActive2(final Player p) {
        ...
        if (Sputnik.CooldownAbs.containsKey(p.getUniqueId())) {
            final boolean isActivable = Sputnik.CooldownAbs.get(p.getUniqueId());
            if (!isActivable) {
                Sputnik.CooldownAbs.put(p.getUniqueId(), true);
    ...
    public static String buildcustomString(final String entityname, final Integer level, final boolean hideLVL) {
        ...
        String returnstring = trans("&8[&7Lvl" + level + "&8] &c" + entityname);
        if (hideLVL) {
        ...
        new BukkitRunnable() {
            public void run() {
                ...
                for (final Player p : loc.getWorld().getPlayers()) {
                    if (!esp.getPlayerUUIDs().contains(p.getUniqueId())) {
                        esp.addPlayer(p);
                    }
                }
            }
