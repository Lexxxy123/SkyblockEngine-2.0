...
public class Region
{
    ...
    public static Region getRegionOfBlock(final Block block) {
        ...
        possible.sort(Comparator.<Region>comparingInt(r -> r.getType().ordinal()));
        Collections.reverse(possible);
        return (possible.size() != 0) ? possible.get(0) : null;
    }
    ...
    public Location getRandomAvailableLocation() {
        ...
        return possible.isEmpty() ? null : SUtil.<Location>getRandom(possible);
    }
    ...
    public List<Integer> getBounds() {
        ...
        return Arrays.<Integer>asList(sx, ex, sy, ey, sz, ez);
    }
    
    public List<Entity> getPlayersWithinRegion() {
        final List<Entity> entities = new ArrayList<Entity>(this.firstLocation.getWorld().getEntitiesByClasses(new Class[] { Player.class }));
