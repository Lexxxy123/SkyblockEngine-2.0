    ...
    public void onOpen(final GUIOpenEvent e) {
        ...
        this.set(new GUIClickableItem() {
            ...
            public ItemStack getItem() {
                ...
                if (bids.size() == 0) {
                    lore.add((Object)ChatColor.GRAY + "You don't have any");
                    lore.add((Object)ChatColor.GRAY + "outstanding bids.");
                    lore.add(" ");
                    lore.add((Object)ChatColor.GRAY + "Use the " + (Object)ChatColor.GOLD + "Auctions Browser");
                    lore.add((Object)ChatColor.GRAY + "to find some cool items.");
                } else if (bids.size() == 1 && ((AuctionItem)bids.get(0)).getTopBidder().getUuid().equals(user.getUuid())) {
                    lore.add((Object)ChatColor.GRAY + "You have the " + (Object)ChatColor.GREEN + "top bid" + (Object)ChatColor.GRAY + " on");
                    lore.add((Object)ChatColor.GRAY + "a pending auction.");
                } else {
                    lore.add((Object)ChatColor.GRAY + "You placed " + (Object)ChatColor.GREEN + bids.size() + " bid" + (bids.size() != 1 ? "s" : "") + (Object)ChatColor.GRAY + " on");
                    lore.add((Object)ChatColor.GRAY + "pending auctions.");
                    lore.add(" ");
                    lore.add((Object)ChatColor.GRAY + "You hold the top bid on");
                    lore.add((Object)ChatColor.GRAY + "(" + (Object)color + held + (Object)ChatColor.GRAY + "/" + (Object)color + bids.size() + (Object)ChatColor.GRAY + ") of these");
                    lore.add((Object)ChatColor.GRAY + "auctions.");
                }
                for (AuctionItem item2 : bids) {
                    if (!item2.getTopBidder().getUuid().equals(user.getUuid()) || !item2.isExpired()) continue;
                    lore.add(" ");
                    lore.add((Object)ChatColor.AQUA + "* You have items/coins to pickup!");
                    break;
                }
                if (bids.size() > 0) {
                    lore.add(" ");
                    lore.add((Object)ChatColor.YELLOW + "Click to view!");
                }
                return SUtil.getStack((Object)(bids.size() == 0 ? ChatColor.YELLOW : ChatColor.GREEN) + "View Bids", Material.GOLDEN_CARROT, (short)0, 1, lore);
            }
        });
