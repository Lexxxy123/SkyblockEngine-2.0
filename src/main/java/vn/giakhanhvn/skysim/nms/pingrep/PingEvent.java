...
public class PingEvent {
    ...
    public ServerInfoPacket createNewPacket(final PingReply reply) {
        try {
            ...
            final Class<?> packet = Class.forName("vn.giakhanhvn.skysim.nms.pingrep." + version + ".ServerInfoPacketHandler");
    ...
    public void sendPong() {
        try {
            final String name = Bukkit.getServer().getClass().getPackage().getName();
            final String version = name.substring(name.lastIndexOf(46) + 1);
            final Class<?> packet = Class.forName("vn.giakhanhvn.skysim.nms.pingrep." + version + ".PongPacketHandler");
