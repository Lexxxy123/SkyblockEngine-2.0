...
public class SlayerBossType
{
    ...
    SlayerBossType(final String namespace, final String name, final SEntityType specType, final int tier, final String description, final SlayerMobType type, final int health, final int dps, final int tdps, final int rewardXP, final int spawnXP, final int cost, final SlayerAbility... abilities) {
        ...
        this.cost = cost;
        this.abilities = Arrays.<SlayerAbility>asList(abilities);
    ...
    public static int staticGetXPReqForLevel(final int level, final EntityType type) {
        switch (type) {
            case ZOMBIE:
                return SUtil.<Integer>getOrDefault((List<Integer>)Arrays.asList(5, 15, 200, 1000, 5000, 20000, 100000, 400000, 1000000), level, 1000000);
            case SPIDER:
                return SUtil.<Integer>getOrDefault((List<Integer>)Arrays.asList(5, 25, 200, 1000, 5000, 20000, 100000, 400000, 1000000), level, 1000000);
            case WOLF:
                return SUtil.<Integer>getOrDefault((List<Integer>)Arrays.asList(10, 30, 250, 1500, 5000, 20000, 100000, 400000, 1000000), level, 1000000);
            case ENDERMAN:
                return SUtil.<Integer>getOrDefault((List<Integer>)Arrays.asList(10, 30, 250, 1500, 5000, 20000, 100000, 400000, 1000000), level, 1000000);
            default:
                return 1000000;
    ...
    static {
        ...
        LIFE_DRAIN = new SlayerAbility(ChatColor.RED + "Life Drain", new String[] { "Drains health every few seconds." });
        PESTILENCE = new SlayerAbility(ChatColor.GREEN + "Pestilence", new String[] { "Deals AOE damage every second,", "shredding armor by 25%." });
        ENRAGE = new SlayerAbility(ChatColor.RED + "Enrage", new String[] { "Gets real mad once in a while." });
        COMBAT_JUMP = new SlayerAbility(ChatColor.YELLOW + "Combat Jump", new String[] { "The spider will often attempt to", "jump behind you." });
        NOXIOUS = new SlayerAbility(ChatColor.RED + "Noxious", new String[] { "Deals AOE damage every second,", "reducing your healing by 50%." });
        AGILE = new SlayerAbility(ChatColor.GREEN + "Agile", new String[] { "The wolf is small and fast, making", "it hard to hit." });
        TRUE_DAMAGE = new SlayerAbility(ChatColor.WHITE + "True Damage", new String[] { "Ignores your defense. Very painful." });
        CALL_THE_PUPS = new SlayerAbility(ChatColor.AQUA + "Call the pups!", new String[] { "At 50% health, calls its deadly pack", "of pups." });
        DISSONANCE = new SlayerAbility(ChatColor.RED + "Dissonance", new String[] { "Once in a while, teleport behind ", "or to the sides of the player." });
        HEAL = new SlayerAbility(ChatColor.GREEN + "Redemption", new String[] { "Heals rapidly." });
        EXPLOSION = new SlayerAbility(ChatColor.RED + "Explosive Assault", new String[] { "Throws explosive TNT." });
        ONEHIT = new SlayerAbility(ChatColor.DARK_PURPLE + "Thermonuclear", new String[] { "Charges up and releases a massive", "explosion." });
        COMING_SOON = new SlayerAbility(ChatColor.RED + "Coming Soon!", new String[] { "More abilities coming soon!" });
        NUKEKUBI = new SlayerAbility(ChatColor.YELLOW + "Nukekubi Fixations", new String[] { "Spawn weird heads.", "Clear them by looking at them or", "suffer damage." });
        YANG_GLYPHS = new SlayerAbility(ChatColor.RED + "Yang Glyphs", new String[] { "Throw glyphs down.", "Stand next to them or die." });
        HITSHIELD = new SlayerAbility(ChatColor.GREEN + "Malevolent Hitshield", new String[] { "Immunity shield dropped by hits", "regardless of damage." });
