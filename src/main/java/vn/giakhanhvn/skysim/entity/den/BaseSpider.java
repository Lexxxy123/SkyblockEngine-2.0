...
public abstract class BaseSpider implements EntityStatistics, EntityFunction
{
    ...
    public void onDeath(final SEntity sEntity, final Entity killed, final Entity damager) {
        ...
        if (quest.getType().getName() == "Tarantula Broodfather") {
            ...
            if (SUtil.random(0, 10) == 0 && quest.getType().getTier() >= 3 && quest.getType().getTier() < 4) {
                ...
                SUtil.delay(() -> new SEntity(k, SEntityType.TARANTULA_VERMIN).setTarget((LivingEntity)player), 12L);
                return;
            }
            if (SUtil.random(0, 18) == 0 && quest.getType().getTier() >= 4) {
                SlayerQuest.playMinibossSpawn(k, (Entity)player);
                SUtil.delay(() -> new SEntity(k, SEntityType.TARANTULA_BEAST).setTarget((LivingEntity)player), 12L);
                return;
            }
            if (SUtil.random(0, 50) == 0 && quest.getType().getTier() >= 4) {
                SlayerQuest.playMinibossSpawn(k, (Entity)player);
                SUtil.delay(() -> new SEntity(k, SEntityType.MUTANT_TARANTULA).setTarget((LivingEntity)player), 12L);
            }
        }
    }
    ...
    public List<EntityDrop> drops() {
        return Arrays.<EntityDrop>asList(new EntityDrop(SMaterial.STRING, EntityDropType.GUARANTEED, 1.0), new EntityDrop(SMaterial.SPIDER_EYE, EntityDropType.OCCASIONAL, 0.5));
    }
}
