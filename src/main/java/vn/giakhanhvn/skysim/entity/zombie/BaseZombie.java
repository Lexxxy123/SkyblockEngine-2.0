    ...
    public void onDeath(final SEntity sEntity, final Entity killed, final Entity damager) {
        ...
        if (quest.getType().getName() == "Revenant Horror" || quest.getType().getName() == "Atoned Horror") {
            ...
            if (SUtil.random(0, 8) == 0 && quest.getType().getTier() >= 3 && quest.getType().getTier() < 4) {
                ...
                SUtil.delay(() -> new SEntity(k, SEntityType.REVENANT_SYCOPHANT).setTarget((LivingEntity)player), 12L);
                return;
            }
            if (SUtil.random(0, 16) == 0 && quest.getType().getTier() == 4) {
                SlayerQuest.playMinibossSpawn(k, (Entity)player);
                SUtil.delay(() -> new SEntity(k, SEntityType.REVENANT_CHAMPION).setTarget((LivingEntity)player), 12L);
                return;
            }
            if (SUtil.random(0, 45) == 0 && quest.getType().getTier() == 4) {
                SlayerQuest.playMinibossSpawn(k, (Entity)player);
                SUtil.delay(() -> new SEntity(k, SEntityType.DEFORMED_REVENANT).setTarget((LivingEntity)player), 12L);
            }
            if (SUtil.random(0, 16) == 0 && quest.getType().getTier() == 5) {
                SlayerQuest.playMinibossSpawn(k, (Entity)player);
                SUtil.delay(() -> new SEntity(k, SEntityType.ATONED_CHAMPION).setTarget((LivingEntity)player), 12L);
            }
            if (SUtil.random(0, 40) == 0 && quest.getType().getTier() == 5) {
                SlayerQuest.playMinibossSpawn(k, (Entity)player);
                SUtil.delay(() -> new SEntity(k, SEntityType.ATONED_REVENANT).setTarget((LivingEntity)player), 12L);
            }
        }
    }
}
