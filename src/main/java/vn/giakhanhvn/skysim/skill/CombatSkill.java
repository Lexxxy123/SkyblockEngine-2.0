...
public class CombatSkill extends Skill
{
    ...
    public List<String> getDescription() {
        return Arrays.<String>asList("Fight mobs and players to earn", "Combat XP!");
    }
    
    @Override
    public List<String> getLevelUpInformation(final int level, final int lastLevel, final boolean showOld) {
        return Arrays.<String>asList(ChatColor.WHITE + " Deal " + (showOld ? (ChatColor.DARK_GRAY + "" + lastLevel * 4 + "➜") : "") + ChatColor.GREEN + level * 4 + "% " + ChatColor.WHITE + "more damage to mobs.", ChatColor.DARK_GRAY + "+" + ChatColor.GREEN + "0.5% " + ChatColor.BLUE + "☣ Crit Chance");
    }
